...
export async function POST(req: NextRequest) {
  try {
    ...
    const currentCount = existingStreak?.streak_count ?? 0;
    let streakBefore: number = currentCount;
    let streakAfter: number = currentCount;
    ...
    if (existingStreak && existingStreak.last_updated_date === todayJst) {
      ...
      streakBefore = currentCount;
      streakAfter = isKnown ? currentCount + 1 : 0;
    }
