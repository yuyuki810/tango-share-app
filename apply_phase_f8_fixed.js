...
// 1. components/review/WordJudgeCardScreen.tsx (古い自前結果画面を完全撤去し、誤爆を根絶)
writeFile(
...
// 2. components/dashboard/TodayRangeCard.tsx (ランダム出題トグルUI & 本番+5点ボーナス)
writeFile(
...
// 3. components/weakness/DrillFilterDialog.tsx (弱点マップドリルでのランダムトグル)
writeFile(
...
// 4. app/(main)/test/page.tsx (戻り先動的制御 & シャッフル)
writeFile(
...
// 5. components/test/TestSessionRunner.tsx
writeFile(
...
// 6. app/api/test-sessions/start/route.ts
writeFile(
...
// 7. app/api/test-sessions/complete/route.ts
writeFile(
...
// 8. app/(main)/group/page.tsx
const groupPath = path.join(__dirname, 'app/(main)/group/page.tsx');
...
if (!groupContent.includes('random_bonus_applied')) {
  ...
  writeFile('app/(main)/group/page.tsx', groupContent);
}

console.log('\n✅ [完了] フェーズF-8（ランダム出題、0%画面バグ根絶、弱点マップ戻り先改善）をすべて適用しました！');
