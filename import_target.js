/**
 * import_target.js
 * ターゲット1900 CSV一括インポートスクリプト（ルート配置用）
 */
...
async function main() {
  // 引数で指定されたファイル、またはカレントディレクトリの target1900.csv を探索
  let targetFile = process.argv[2] || 'target1900.csv';
  let fullPath = path.isAbsolute(targetFile) ? targetFile : path.join(process.cwd(), targetFile);

  if (!fs.existsSync(fullPath)) {
    // 拡張子なし等も探索
    const candidates = ['target1900.csv', 'target_1900.csv', 'target.csv'];
  ...
  if (!fs.existsSync(fullPath)) {
    ...
    console.log('💡 ファイル名を確認して `node import_target.js [ファイル名.csv]` で実行してください。');
    process.exit(1);
  }
  ...
  let numIdx = headerRow.findIndex((h) => ['no', 'number', 'id', '番号', 'no.', '連番'].includes(h));
  ...
  if (wordIdx === -1 || meaningIdx === -1) {
    console.log('ℹ️ ヘッダー行が検出されないため、[番号, 単語, 意味, (発音)] として解析します。');
    dataRows = rows;
    numIdx = 0;
    wordIdx = 1;
    meaningIdx = 2;
  ...
  console.log(`📊 検出されたデータ行数: ${dataRows.length} 語`);

  // 4. 単語帳 (wordbooks) レコードの作成または更新
  ...
  if (existingWb) {
    ...
    console.log(`📖 既存の単語帳レコードを更新します: "${wordbookName}" (ID: ${wordbookId})`);
    ...
    wordbookId = newWb.id;
    console.log(`📖 新規単語帳レコードを作成しました: "${wordbookName}" (ID: ${wordbookId})`);
  }
  ...
  for (let i = 0; i < wordsToInsert.length; i += BATCH_SIZE) {
    ...
    if (insertError) {
      console.error(`\n❌ バッチ登録エラー (${i + 1}〜${i + batch.length}語):`, insertError.message);
    } else {
  ...
  console.log('================================================================');
  console.log('💡 アプリの「単語帳設定（/settings/wordbook）」から「ターゲット1900」を選択して学習を開始できます。');
}
